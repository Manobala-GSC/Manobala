...
function AdminDashboard() {
  ...
  const [pendingExpertUserId, setPendingExpertUserId] = useState(null)
  ...
  const handleToggleExpert = async (userId) => {
    ...
    if (!userToUpdate.isExpert) {
      // Making someone an expert — show specialization picker
      setPendingExpertUserId(userId)
      setSelectedSpecialization("")
      setShowSpecModal(true)
    } else {
      // Removing expert status — just toggle off
      try {
        ...
        if (data.success) {
          setUsers(users.map((user) => (user._id === userId ? { ...user, isExpert: false, specialization: '' } : user)))
          toast.success("User is no longer an expert")
        }
      } catch (error) {
  ...
  const confirmMakeExpert = async () => {
    ...
    try {
      const { data } = await axios.patch(
        `${backendUrl}/api/admin/users/${pendingExpertUserId}`,
        {
          isExpert: true,
      ...
      if (data.success) {
        setUsers(
          users.map((user) =>
            user._id === pendingExpertUserId
              ? { ...user, isExpert: true, specialization: selectedSpecialization }
              : user,
          ),
        )
        toast.success(`User is now an expert (${selectedSpecialization})`)
      }
    } catch (error) {
      toast.error(error.response?.data?.message || "Error updating expert status")
    } finally {
      setShowSpecModal(false)
      setPendingExpertUserId(null)
      setSelectedSpecialization("")
    }
  }
                  ...
                  {users.map((user) => (
                    <tr key={user._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {user.isExpert && user.specialization && (
                          <span className="block mt-1 text-xs text-purple-600">{user.specialization}</span>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
      ...
      {showSpecModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
            <h3 className="text-lg font-bold text-primary mb-2">Select Specialization</h3>
            <p className="text-sm text-gray-500 mb-4">
              Choose a specialization for this expert before confirming.
            </p>
              ...
              {specializations.map((spec) => (
                ...
                >
                  ...
                  <span className={`text-sm font-medium ${
                    selectedSpecialization === spec ? "text-primary" : "text-gray-700"
                  }`}>
                    {spec}
                  </span>
                </label>

  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      if (error.response?.status === 429) {
        errorText = "⚠️ Rate limit exceeded. You've sent too many requests. Please wait a minute and try again."
      } else if (error.response?.status === 400) {
        errorText = "⚠️ Invalid API key. Please check your VITE_GEMINI_API_KEY in the .env file. The key should start with 'AIzaSy'."
      } else if (error.response?.status === 403) {
        errorText = "⚠️ API key is restricted or disabled. Please generate a new key from Google AI Studio."
      }
